...
public final class FaceDetectRGBActivity extends AppCompatActivity implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    public void onCreate(Bundle icicle) {
        ...
        Button btnCameraRGB = (Button) findViewById(R.id.btnRec);
        btnCameraRGB.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(FaceDetectRGBActivity.this, UserInfoActivity.class);
    ...
    private class FaceDetectThread extends Thread {
        ...
        private opencv_objdetect.CascadeClassifier detector;

        public FaceDetectThread(Handler handler, Context ctx) {
            this.ctx = ctx;
            this.handler = handler;
            //this.detector = TrainHelper.loadClassifierCascade(ctx, R.raw.frontalface);
        }
                    ...
                    if (rect.height() * rect.width() > 100 * 100) {
                        ...
                        if (facesCount.get(idFace) == null) {
                            ...
                            if (count == 2) {
                                ...
                                System.out.println("facecrop宽度"+faceCroped.getWidth());
                                System.out.println("facecrop高度"+faceCroped.getHeight());

                                if (faceCroped != null) {
                                    final String facebase64 = BitmapUtil.bitmapZoomToBase64(faceCroped,0.1f);
                                    System.out.println("face64"+facebase64);
                                    handler.post(new Runnable() {
                                        public void run() {
                                            imagePreviewAdapter.add(faceCroped);
                                            Intent intent = new Intent(FaceDetectRGBActivity.this, UserInfoActivity.class);
                                            IntentEnvoy.getInstance().deliver(IntentEnvoy.ENVOY_IMAGE_BASE64_KEY,facebase64);                           //intent.putExtra("facebase64",facebase64);
                                            startActivity(intent);
                                        }
                                    });
                                }
